...
When we have two variables measured per "unit" (e.g., measure height and weight for each person), we can refer to this as "bivariate" data.  We already discussed how to analyze [contingency tables](nhst-chi-squared.html) when dealing with bivariate categorical data, here we are concerned with bivariate numerical data that may have a *linear relationship*.
...
Here are a few more [detailed notes on the covariance](bivariate-covariance.html).
...
For more details on the correlation, here are some [detailed notes](bivariate-corelation.html).
...
In the [detailed notes](bivariate-ols.html) we go through the mathematical definitions, but here, let's just sort out what the different words in the sentence above mean.
...
We have some detailed notes on [partitioning variance](bivariate-determination.html), but briefly.  When we partition variance, we mostly just consider the partitioning of the "sums of squares", which we can get in R via the `anova` (analysis of variance) command, which tells us the variability in y attributable to our explanatory variable, and that which is left unexplained (residuals).  
...
(In this father-son data, all these p-values are so small, that they are past the limit of our computer's ability to represent tiny numbers.  Thus, even if they were different, we wouldn't be able to tell.  To see a case in which they are less tiny, [look here](bivariate-significance.html).
...
This interval is much broader because, although we can estimate the best fitting line (and thus the mean) very well, the individual heights have a lot of variability around that line.  For more on these intervals, [read here](bivariate-prediction.html)
