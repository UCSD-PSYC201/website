...
```{r}
p.legit.given.neg <- p.legit.neg / p.neg
p.legit.given.neg <- (1 - p.pos.given.legit) * (1 - p.drug) / p.neg
p.legit.given.neg <- (1 - p.pos.given.legit) * (1 - p.drug) / (p.neg.given.drug * p.drug + (1 - p.pos.given.legit) * (1 - p.drug))

p.legit <- 1 - p.drug
p.neg.given.legit <- 1 - p.pos.given.legit
p.neg <- p.neg.given.legit * p.legit + p.neg.given.drug * p.drug
...
```{r}
my.binom <- function(n) choose(10, n) * p.head^n * (1 - p.head)^(10 - n)
my.binom(0:10)
tibble(x = 0:10, p = my.binom(x)) %>% 
  ggplot(aes(x, p)) +
...
```{r}
dbinom(0:10, 10, p.head)
tibble(x = 0:10, p = dbinom(x, 10, p.head)) %>% 
  ggplot(aes(x, p)) +
  geom_bar(stat = 'identity')
```
...
```{r}
my.cdf <- Vectorize(function(n) sum(my.binom(0:n)))
my.cdf(0:10)

my.cdf2 <- cumsum(my.binom(0:10))
tibble(x = 0:10, p = my.cdf(x)) %>% 
  ggplot(aes(x, p)) +
  geom_bar(stat = 'identity')
```
...
```{r}
tibble(x = 0:10, p = pbinom(x, 10, p.head)) %>% 
  ggplot(aes(x, p)) +
  geom_bar(stat = 'identity')
```
