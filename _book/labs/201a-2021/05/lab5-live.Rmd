...
```{r}
mytest <- with(iris, cor.test(Petal.Length, Petal.Width))
mytest$p.value
```
...
```{r}
model1 <- lm(Sepal.Length ~ Petal.Length, iris)
...
```{r}
t <- summ1$coefficients[2, 3]
p <- summ1$coefficients[2, 4]
```
...
```{r}
summary(lm(Sepal.Length ~ Petal.Length, iris %>% filter(Species == 'setosa')))
...
map(
  group_split(grouped),
  function(df) summary(lm(Sepal.Length ~ Petal.Length, df))
)
```
...
```{r}
predicted <- as_tibble(predict(model1, fabricola, interval = 'prediction'))
```
...
```{r}
fab2 <- bind_cols(fabricola, predicted)
iris %>% 
  ggplot(aes(Petal.Length, Sepal.Length)) +
  geom_point(aes(color = Species), alpha = .5) +
  geom_ribbon(aes(y = fit, ymin = lwr, ymax = upr), fab2, alpha = .3) +
  geom_point(aes(y = fit), fab2, alpha = .5)
```
