...
```{r}
iris %>% with(cor(Petal.Length, Petal.Width))
```

Is this correlation significant?  Use `cor.test`.

```{r}
iris %>% with(cor.test(Petal.Length, Petal.Width))
```
...
```{r}
model$coefficients
coeff <- lm.sum$coefficients
```

Extract the t-statistics and p-values of the slope from the `lm` model.

```{r}
coeff[2, 3]
```

Use `pt` to verify that the t-statistics and p-values match.

```{r}
lm.sum
pt(-abs(coeff[2, 3]), nrow(iris) - 2) * 2
```
...
iris %>%
  group_by(Species) %>%
  group_split() %>%
  map(~ summary(lm(Sepal.Length ~ Petal.Length, .)))
```
...
```{r}
pred <- predict(model, fabricola, interval = "prediction")
```
...
iris %>%
  ggplot(aes(Petal.Length, Sepal.Length)) +
  geom_point(aes(color = Species), alpha = .5) +
  geom_ribbon(aes(ymin = lwr, ymax = upr), fab2, alpha = .3) +
  geom_point(aes(color = Species), fab2, alpha = .5)
```
